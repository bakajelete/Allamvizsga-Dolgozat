...
class xPage02(Page.Page):
    def __init__(self, root, page_manager):
        ...
        self.analize_02 = Button(self, text="SMA crosses")
    ...
    def show_widgets(self):
        ...
        self.all_companies_info = pd.read_csv("E:/SZAKDOLGOZAT/A_DOLOGZAT/system/Companies_Information.csv")###
    ...
    def change_vizualization(self, event, btn, period):
        if period > self.data_time_size:
            messagebox.showerror('Error', 'Error: Not enough data is loaded!')
            return
    ...
    def refresh_company_info_labeles(self):
        ...
        row = self.all_companies_info[self.all_companies_info['Symbol'] == ticker]
    ...
    def resize_window(self, event):
        ...
        self.max_time.config(font=("Helvetica bold", int(self.x_font)))
                ...
                #_____Company info labels_____
        self.sector.place(x=self.x_len-4*self.box_x, y=2*self.box_y, width=4*self.box_x, height=1*self.box_y)
        self.sector.config(font=("Helvetica bold", int(self.x_font)))
        
        self.sector_inf.place(x=self.x_len-4*self.box_x, y=3*self.box_y, width=4*self.box_x, height=1*self.box_y)
        self.sector_inf.config(font=("Helvetica bold", int(self.x_font)))

        self.industry.place(x=self.x_len-4*self.box_x, y=4*self.box_y, width=4*self.box_x, height=1*self.box_y)
        self.industry.config(font=("Helvetica bold", int(self.x_font)))
        
        self.industry_inf.place(x=self.x_len-4*self.box_x, y=5*self.box_y, width=4*self.box_x, height=1*self.box_y)
        self.industry_inf.config(font=("Helvetica bold", int(self.x_font)))

        self.market_cap.place(x=self.x_len-4*self.box_x, y=6*self.box_y, width=4*self.box_x, height=1*self.box_y)
        self.market_cap.config(font=("Helvetica bold", int(self.x_font)))
        
        self.market_cap_inf.place(x=self.x_len-4*self.box_x, y=7*self.box_y, width=4*self.box_x, height=1*self.box_y)
        self.market_cap_inf.config(font=("Helvetica bold", int(self.x_font)))
        
        self.country.place(x=self.x_len-4*self.box_x, y=8*self.box_y, width=4*self.box_x, height=1*self.box_y)
        self.country.config(font=("Helvetica bold", int(self.x_font)))

        self.country_inf.place(x=self.x_len-4*self.box_x, y=9*self.box_y, width=4*self.box_x, height=1*self.box_y)
        self.country_inf.config(font=("Helvetica bold", int(self.x_font)))
        
                #_____Buttons for page change_____
        self.more_details.place(x=4*self.box_x, y=17*self.box_y, width=4*self.box_x, height=1*self.box_y)
        self.more_details.config(font=("Helvetica bold", int(self.x_font)))
        
        self.analize_01.place(x=9*self.box_x, y=17*self.box_y, width=4*self.box_x, height=1*self.box_y)
        self.analize_01.config(font=("Helvetica bold", int(self.x_font)))
        
        self.analize_02.place(x=14*self.box_x, y=17*self.box_y, width=4*self.box_x, height=1*self.box_y)
        self.analize_02.config(font=("Helvetica bold", int(self.x_font)))
        
        self.analize_03.place(x=19*self.box_x, y=17*self.box_y, width=4*self.box_x, height=1*self.box_y)
        self.analize_03.config(font=("Helvetica bold", int(self.x_font)))
        
        self.analize_04.place(x=24*self.box_x, y=17*self.box_y, width=4*self.box_x, height=1*self.box_y)
        self.analize_04.config(font=("Helvetica bold", int(self.x_font)))
        
        self.analize_05.place(x=29*self.box_x, y=17*self.box_y, width=4*self.box_x, height=1*self.box_y)
        ...
        self.plots.get_tk_widget().place(x=3*self.box_x, y=2*self.box_y, width=28*self.box_x, height=14*self.box_y)

                #_____Label for information of the graph_____
        self.graph_info.place(x=3*self.box_x, y=16*self.box_y, width=28*self.box_x, height=1*self.box_y)
