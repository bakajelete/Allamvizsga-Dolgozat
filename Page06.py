...
class xPage06(Page.Page):
    def __init__(self, root, page_manager):
        ...
        self.degree = Label(self, text="Degree of polynom:", anchor="center")
        ...
        self.calculate_polynom = Button(self, text="Calculate polynom", anchor="center")
    ...
    def calc_polynomial_regression(self, event):
        ...
        if degree == 0:
            messagebox.showerror('Python Error', 'Error: Polynomial degree can\'t be 0!')
            return

        if degree > n:
            messagebox.showerror('Python Error', 'Error: Polynomial degree can\'t be bigger than the length of the loaded data!')
            return
        ...
        if degree > n/2:
            messagebox.showwarning('WARNING!', 'Warning: The Polynomial degree is too large, might cause over fitting!')
        
        if degree < n/5:
            messagebox.showwarning('WARNING!', 'Warning: The Polynomial degree is too small, might cause under fitting!')
            
        if delta > n/10:
            messagebox.showwarning('WARNING!', 'Warning: The prediction delta is too large, might cause exponential prediction error!')            

        x_tr = np.linspace(0,n-1,n)
        ...
        with warnings.catch_warnings():
            ...
            y_test = [model(i) for i in range(n)]
            y_pred = [model(i) for i in range(n,n+delta)]
        ...
        self.subplot.plot(pred_dates, y_pred[-(delta+1):], color = pred_color)
