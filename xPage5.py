...
class Page2:
    ...
    def load_in_historical_data(self, begin, end):
        ...
        historical_row = self.historical_data[begin].strip().split(',')
        num = historical_row[1]
        num_flo = float(num)
        
        highest_high = num_flo
        lowes_low = num_flo
    ...
    def change_initial_vizualization(self):
        ...
        self.subplot.clear()
        min_val, max_val, x, y, trend_model, dif, color, information = self.build_up_Trend_model(self.max_time-6, self.max_time)
