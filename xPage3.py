...
class Page3:
    ...
    def __init__(self, root):
        ...
        self.one_month_changes = Button(root, text="One month changes", command = lambda: self.change_vizualization_changes(self, self.get_days_nr(self.now - relativedelta(months=1))))
        ...
        self.three_months_changes = Button(root, text="Three months changes", command = lambda: self.change_vizualization_changes(self, self.get_days_nr(self.now - relativedelta(months=3))))
        ...
        self.six_months_changes = Button(root, text="Six months changes", command = lambda: self.change_vizualization_changes(self, self.get_days_nr(self.now - relativedelta(months=6))))
        ...
        self.one_year_changes = Button(root, text="One year changes", command = lambda: self.change_vizualization_changes(self, self.get_days_nr(self.now - relativedelta(years=1))))
        ...
        self.three_years_changes = Button(root, text="Three years changes", command = lambda: self.change_vizualization_changes(self, self.get_days_nr(self.now - relativedelta(years=3))))
        ...
        self.five_years_changes = Button(root, text="Five years  changes", command = lambda: self.change_vizualization_changes(self, self.get_days_nr(self.now - relativedelta(years=5))))

        self.max_time = Button(root, text="Max time", command = lambda: self.change_vizualization(self, self.data_time_size))
        self.max_time_changes = Button(root, text="Max time  changes", command = lambda: self.change_vizualization_changes(self, self.data_time_size))
        ...
        self.date_start = DateEntry(self.root, popanchor="n", date_pattern='YYYY-MM-DD') #, width= 16, date_pattern='YYYY-MM-DD', background= "magenta3", foreground= "white",bd=2)
        

        self.date_to = Label(root, text="To:", anchor="center")
        self.date_end = DateEntry(self.root, calendar_position='above', date_pattern='YYYY-MM-DD') #, width = 16, date_pattern='YYYY-MM-DD', background= "magenta3", foreground= "white",bd=2)
    ...
    def new_vizualization_changes(self, period):
        ...
        sizes = [abs(x) * self.x_font * 0.5  for x in percentage_changes['Open']]
        ...
        percentage_changes['Date'] = pd.to_datetime(percentage_changes['Date'], format='%Y-%m-%d')
    ...
    def resize_window(self, event):
        ...
        self.show.place(x=22*self.box_x, y=1*self.box_y, width=4*self.box_x, height=1*self.box_y)
